...
from resources.errors import SchemaValidationError, TaskAlreadyExistsError, InternalServerError, \
UpdatingTaskError, DeletingTaskError, TaskNotExistsError
...
class TaskApi(Resource):
    ...
    def put(self, id):
        try:
            ...
            #task.update(**body)
            print(body)
            Task.objects(id=id, added_by=user_id).update(name=body['name'], developpers=body['developpers'], type=body['type'])
            return '', 200
    ...
    def delete(self, id):
        try:
            ...
            task = Task.objects.get(id=id, added_by=user_id)
            task.delete()
            return 'succssefuly deleted', 200
        except DoesNotExist:
